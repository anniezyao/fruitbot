...
POSTERIOR: float = POSTERIOR_DEFAULT  # updated each iteration from file
MIN_TOTAL_QTY = 400.0
EPS = 1e-9
_FAIR_EMA: float = POSTERIOR  # module-level EMA of market signal
K = 0.05
FIT_MIN = 0.10
FIT_MAX = 0.85
FIT_EMA_LAMBDA = 0.05
...
def _read_posterior_from_file(path: str = APPLE_THEO_PATH) -> float:
    """Read posterior (theoretical fair anchor) from a plain-text file.

    The file should contain a single number (int/float). If missing or invalid,
    fall back to the current POSTERIOR.
    """
    global POSTERIOR
    try:
...
def calc_fair(fit):
    ...
    tas = get_tas('APPL', TRADE_WINDOW_TICKS) or []
    if not tas:
        fair = POSTERIOR
...
class BotState:
    fit: float = FIT_MIN
    last_posterior: float = POSTERIOR
...
def step(state: BotState, cfg: BotConfig) -> Tuple[BotState, Optional[QuotePlan], Optional[float]]:
    ...
    if new_post != POSTERIOR:
        POSTERIOR = new_post
    if POSTERIOR != state.last_posterior:
        _FAIR_EMA += (POSTERIOR - state.last_posterior)
        state.last_posterior = POSTERIOR
    snap = observe(cfg)
